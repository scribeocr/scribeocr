...
export async function convertPageHocr({ocrStr, n, pageDims = null, rotateAngle = 0, keepBold = false}) {
  ...
  if(!pageDims) {
    let pageElement = ocrStr.match(/<div class=[\"\']ocr_page[\"\'][^\>]+/i);
    if (pageElement != null) {
  ...
  const charMode = /ocrx_cinfo/.test(ocrStr) ? true : false;
...
export async function convertPageAbbyy({ocrStr, n}) {
  ...
  let pageDimsStr = ocrStr.match(/<page width=[\'\"](\d+)[\'\"] height=[\'\"](\d+)[\'\"]/);
...
function convertTableLayoutAbbyy(ocrStr) {
  ...
  let tables = ocrStr.match(tableRegex);
