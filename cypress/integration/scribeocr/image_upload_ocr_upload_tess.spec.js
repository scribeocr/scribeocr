...
describe('Produces output files from image upload and no OCR data for', () => {
  ...
  it('pdf with no imported ocr data, produces download files in supported formats (.txt, .docx, .pdf, and .hocr)', () => {
    cy.get('#openFileInput').selectFile(['cypress/fixtures/multi_pdf_nd/aurelia.pdf', 'cypress/fixtures/multi_png/aurelia_tess.xml'], { force: true })
    cy.waitImport()
  ...
  it('image with no imported ocr data, produces download files in supported formats (.txt, .docx, .pdf, and .hocr)', () => {
    cy.get('#openFileInput').selectFile(['cypress/fixtures/multi_png/aurelia.png', 'cypress/fixtures/multi_png/aurelia_tess.xml'], { force: true })
    cy.waitImport()
