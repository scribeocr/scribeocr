...
describe('Check small caps are detected in PDF imports.', function () {
  ...
  before(async () => {
    await initGeneralScheduler();
    opt.extractText = true;
    await importFilesAll([`${ASSETS_PATH_KARMA}/small_caps_examples.pdf`]);
  });
  ...
  it('Should correctly import small caps printed using small caps font.', async () => {
    assert.strictEqual(ocrAll.active[1].lines[4].words[0].smallCaps, true);

    assert.strictEqual(ocrAll.active[1].lines[4].words.map((x) => x.text).join(' '), 'Abstract');
  }).timeout(10000);
...
describe('Check superscripts are detected in PDF imports.', function () {
  ...
  before(async () => {
    await initGeneralScheduler();
    opt.extractText = true;
    await importFilesAll([`${ASSETS_PATH_KARMA}/superscript_examples.pdf`]);
  });
  ...
  it('Should correctly import trailing superscripts printed using font size adjustments (2nd doc)', async () => {
    assert.strictEqual(ocrAll.active[1].lines[1].words[2].sup, true);
    assert.strictEqual(ocrAll.active[1].lines[1].words[2].text, '1');
  }).timeout(10000);

  it('Should correctly import leading superscripts printed using font size adjustments (2nd doc)', async () => {
    assert.strictEqual(ocrAll.active[1].lines[36].words[0].sup, true);
    assert.strictEqual(ocrAll.active[1].lines[36].words[0].text, '1');
  }).timeout(10000);

  it('Should correctly calculate line angle for lines that start with superscripts (2nd doc)', async () => {
    // Line that starts with superscript.
    assert.strictEqual(ocrAll.active[1].lines[36].baseline[0], 0);
  }).timeout(10000);

  // Third document
  it('Should correctly import leading superscripts printed using font size adjustments (3rd doc)', async () => {
    assert.strictEqual(ocrAll.active[2].lines[24].words[0].sup, true);
    assert.strictEqual(ocrAll.active[2].lines[24].words[0].text, '2');
  }).timeout(10000);

  it('Should correctly parse font size for lines with superscripts (3rd doc)', async () => {
    const words = ocrAll.active[2].lines[24].words;
  ...
  it('Should correctly import trailing superscripts printed using font size adjustments (4th doc)', async () => {
    assert.strictEqual(ocrAll.active[3].lines[114].words[2].sup, true);
    assert.strictEqual(ocrAll.active[3].lines[114].words[2].text, '20');
  }).timeout(10000);

  it('Should correctly parse font size for lines with superscripts (4th doc)', async () => {
    assert.strictEqual(ocrAll.active[3].lines[250].words[1].sup, true);
    assert.strictEqual(ocrAll.active[3].lines[250].words[1].text, '20');
  }).timeout(10000);

  // Fifth document
  it('Should correctly import trailing superscripts printed using font size adjustments (4th doc)', async () => {
    assert.strictEqual(ocrAll.active[4].lines[11].words[16].sup, true);
    assert.strictEqual(ocrAll.active[4].lines[11].words[16].text, '2');
  }).timeout(10000);

  it('Should correctly parse font size for lines with superscripts (4th doc)', async () => {
    assert.strictEqual(ocrAll.active[4].lines[21].words[0].sup, true);
    assert.strictEqual(ocrAll.active[4].lines[21].words[0].text, '2');
  }).timeout(10000);
