...
function parseCmapTableFormat0(cmap, p, platformID, encodingID) {
    ...
    var indexMap = p.parseByteList(cmap.length);
    var glyphIndexMap = Object.assign({}, indexMap);
    var encoding = getEncoding(platformID, encodingID, cmap.language);
    var decodingTable = eightBitMacEncodings[encoding];
    for (var i = 0; i < decodingTable.length; i++) {
...
function parseCmapTable(data, start) {
    ...
    var format14Parser = null;
    var format14offset = -1;
    var offset = -1;
    var platformId = null;
    var encodingId = null;
    var platform0Encodings = [0,1,2,3,4,6];
    var platform3Encodings = [0,1,10];
...
function makeCmapTable(glyphs) {
    ...
    if (!isPlan0Only)
        { cmapTable.push({name: 'cmap12PlatformID', type: 'USHORT', value: 3}, // We encode only for PlatformID = 3 (Windows) because it is supported everywhere
            {name: 'cmap12EncodingID', type: 'USHORT', value: 10},
            {name: 'cmap12Offset', type: 'ULONG', value: 0}); }

    cmapTable.push({name: 'format', type: 'USHORT', value: 4},
    ...
    for (var i$1 = 0; i$1 < endCounts.length; i$1++) {
        t.fields.push(endCounts[i$1]);
    }
    t.fields.push({name: 'reservedPad', type: 'USHORT', value: 0});
    for (var i$2 = 0; i$2 < startCounts.length; i$2++) {
        t.fields.push(startCounts[i$2]);
    }
    for (var i$3 = 0; i$3 < idDeltas.length; i$3++) {
        t.fields.push(idDeltas[i$3]);
    }
    for (var i$4 = 0; i$4 < idRangeOffsets.length; i$4++) {
        t.fields.push(idRangeOffsets[i$4]);
    }
    for (var i$5 = 0; i$5 < glyphIds.length; i$5++) {
        t.fields.push(glyphIds[i$5]);
    }
    ...
    if (!isPlan0Only) {
        ...
        t.fields.push({name: 'cmap12Format', type: 'USHORT', value: 12},
            {name: 'cmap12Reserved', type: 'USHORT', value: 0},
            {name: 'cmap12Length', type: 'ULONG', value: cmap12Length},
            {name: 'cmap12Language', type: 'ULONG', value: 0},
            {name: 'cmap12nGroups', type: 'ULONG', value: cmap12Groups.length / 3});

        for (var i$6 = 0; i$6 < cmap12Groups.length; i$6++) {
            t.fields.push(cmap12Groups[i$6]);
        }
...
function makeGposTable(kerningPairs) {
    ...
    kerningArray.sort(function (a, b) {
        var aLeftGlyph = parseInt(a[0].match(/\d+/)[0]);
        var aRightGlyph = parseInt(a[0].match(/\d+$/)[0]);
        var bLeftGlyph = parseInt(b[0].match(/\d+/)[0]);
        var bRightGlyph = parseInt(b[0].match(/\d+$/)[0]);
    ...
    var nPairs = kerningArray.length;
    ...
    for (var i = 0; i < nPairs; i++) {
        ...
        var firstGlyph = parseInt(kerningArray[i][0].match(/\d+/)[0]);
        var secondGlyph = parseInt(kerningArray[i][0].match(/\d+$/)[0]);
    ...
    for (var i$1 = 0; i$1 < firstGlyphs.length; i$1++) {
        result.fields.push({ name: 'pairSetOffsets', type: 'USHORT', value: offsetN });
        offsetN = offsetN + 2 + 4 * (kerningGlyphs2[i$1].length);
    }

    // Add PairSet tables (one for each first letter in a kerning pair)
    for (var i$2 = 0; i$2 < kerningGlyphs2.length; i$2++) {
        result.fields.push({ name: 'pairValueCount', type: 'USHORT', value: kerningGlyphs2[i$2].length });
        for (var j = 0; j < kerningGlyphs2[i$2].length; j++) {
            result.fields.push({ name: 'secondGlyph', type: 'USHORT', value: kerningGlyphs2[i$2][j][0] });
            result.fields.push({ name: 'valueRecord1', type: 'USHORT', value: kerningGlyphs2[i$2][j][1] });
    ...
    for (var i$3 = 0; i$3 < firstGlyphs.length; i$3++) {
        result.fields.push({ name: 'UppercasePGlyphID', type: 'USHORT', value: firstGlyphs[i$3] });
    }
