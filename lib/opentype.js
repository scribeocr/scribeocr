...
(function (global, factory) {
	...
	encode.DICT = function(m) {
	    ...
	    for (var i = 0; i < length; i += 1) {
			...
			for (const x of enc1) d.push(x);
			for (const x of enc2) d.push(x);
	...
	encode.TABLE = function(table) {
	    ...
	    for (var i$1 = 0; i$1 < subtables.length; i$1 += 1) {
	        ...
	        d[o] = offset >> 8;
			d[o + 1] = offset & 0xff;
			for (const x of subtables[i$1]) d.push(x);
	    }
	...
	function makeOS2Table(options) {
	    return new table.Table('OS/2', [
	        ...
	        {name: 'usMaxContext', type: 'USHORT', value: 2}
	    ], options);
	}
	...
	function fontToSfntTable(font) {
		...
		if (Object.keys(font.kerningPairs).length > 0){
			console.log("Making gpos table.")
	...
	*****/
	...
	function unhex(str) {
	  ...
	  function makeGposTable(kerningPairs) {
		...
		//const kerningGlyphs = Object.keys(kerningPairs);
		const nPairs = kerningArray.length;

		// const firstGlyph = new Array(nPairs);
		// const secondGlyph = new Array(nPairs);

		var firstGlyphs = new Array;
		var kerningGlyphs2 = new Array;

		for(let i = 0; i < nPairs; i++) {

			// firstGlyph[i] = kerningGlyphs[i].match(/\d+/)[0];
			// secondGlyph[i] = kerningGlyphs[i].match(/\d+$/)[0];
			...
			if(firstGlyph != firstGlyphs[firstGlyphs.length - 1]){
				...
				kerningGlyphs2[firstGlyphs.length - 1] = new Array();
			}
	...
	function makeKernTable(kerningPairs) {
		...
		const nPairs = Object.keys(kerningPairs).length;
		// Largest integer n such that 2^n <= nPairs
		const entrySelector = Math.floor(Math.log(nPairs) / Math.log(2));
		// Largest power of 2 <= nPairs
		const highestPowerOf2 = Math.pow(2,entrySelector);
	  
		const searchRange = highestPowerOf2 * 6;
		const rangeShift = (nPairs - highestPowerOf2) * 6;
	  
		// Each kerning pair has 3 values, and the header has 7 values total.
		// Each value is 16 bits (2 bytes).
		const subtableLength = nPairs * 6 + 7 * 2;
	  
		  var result = new table.Table('kern', [
			  {name: 'tableVersion', type: 'USHORT', value: 0},
		  ...
		  for (const [key, value] of Object.entries(kerningPairs)) {
			  console.log(key.match(/\d+/)[0]+ key.match(/\d+$/)[0] + value)
			result.fields.push({name: 'left', type: 'USHORT', value: parseInt(key.match(/\d+/)[0])});
			result.fields.push({name: 'right', type: 'USHORT', value: parseInt(key.match(/\d+$/)[0])});
			result.fields.push({name: 'value', type: 'FWORD', value: value});
		  }
